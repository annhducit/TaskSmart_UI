...
import useGetInviteWorkspace from '../../invite/hooks/mutation/use-invite-workspace';
import SearchParamService from '@/shared/services/search-param';
import useGetWorkspace from '../hooks/query/use-get-workspace';

export default function ModifyMemberWorkspace() {
  return (
    <Dialog.Param
      paramKey={SEARCH_PARAMS.MODAL}
      size='xs'
      paramValue={SEARCH_PARAMS_VALUE.ADD_MEMBER_WORKSPACE}
    >
      <ModalModifyMember />
    </Dialog.Param>
  );
...
const ModalModifyMember = () => {
  ...
  const workspaceId = SearchParamService.useId();

  /**
   * Refresh link when click refresh button
   */
  const { mutate: refresh } = useGetInviteWorkspace({ isPublic: true, refresh: true });
  const { mutate: isPublic } = useGetInviteWorkspace({ isPublic: true, refresh: false });
  const { mutate: isPrivate } = useGetInviteWorkspace({ isPublic: false, refresh: false });

  const { data: workspace } = useGetWorkspace();

  /**
   * Set link when workspace invite code is available for the first time
   */
  useEffect(() => {
    if (workspace?.inviteCode) {
      setLink(`${window.location.origin}/tsm/invite/${workspaceId}/${workspace?.inviteCode}`);
    }
    return;
  }, [workspace]);
  ...
  return (
    <>
      ...
      <div className='flex flex-col gap-y-2'>
        <Typography.Text className='my-2 text-xl font-semibold'>
        ...
        <div className='flex items-center justify-between'>
          {workspace?.inviteCode && <CopyUrlButton link={link} />}
          <div className='flex items-center gap-x-2'>
            <Tooltip
              title={
                workspace?.inviteCode
                  ? 'Anyone with the link can view this workspace'
                  : 'Only workspace members can view this workspace'
              }
            >
              <Switch
                defaultChecked={workspace?.inviteCode !== '' ? true : false}
                size='small'
                onChange={(value) => {
