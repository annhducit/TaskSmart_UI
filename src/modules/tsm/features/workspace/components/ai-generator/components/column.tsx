...
interface Props {
  column: ListCardGenerate;
  cards: CardGenerate[];
  deleteColumn?: (id: string) => void;
  updateColumn?: (column: ListCardGenerate) => void;
  createCard?: (columnId: string, card: CardGenerate) => void;
  updateCard?: (id: Id, content: string) => void;
  deleteCard?: (id: Id) => void;
}

const ColumnContainerAI = ({
  column,
  cards,
  updateColumn,
  deleteColumn,
  ...
  const [cardCreation, setCardCreation] = useState<CardGenerate>({} as CardGenerate);
            ...
            <SortableContext items={tasksIds}>
              {cards.map((card) => (
                <TaskCardAI
                  key={card.id}
                  card={card}
                  deleteTask={deleteCard && (() => deleteCard(card.id))}
                />
              ))}
            </SortableContext>
          </div>
...
export default ColumnContainerAI;
